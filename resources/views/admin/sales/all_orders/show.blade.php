...
{{--
        ...
        <div class="row gutters-5">
            <div class="col text-center text-md-left">
                <address>
                    <strong class="text-main">{{translate('Name').": ". json_decode($order->shipping_address)->name }}</strong><br>
                    {{translate('Phone').": ". json_decode($order->shipping_address)->phone }}<br>
                    {{translate('Address'). ": ". json_decode($order->shipping_address)->address }}, {{translate('City') .": ". json_decode($order->shipping_address)->city }}<br>
                </address>
            ...
            <table class="table">
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            @php
                                $grandTotalWithTax = $order->grand_total;
                                $grandTotalWithoutTax = $grandTotalWithTax - ($grandTotalWithTax * 0);
                            @endphp
                            {{-- {{ single_price($order->orderDetails->sum('tax')) }} --}}
                            {{ single_price($grandTotalWithoutTax * 0) }}
                    ...
                    <tr>
                        ...
                        <td class="text-muted h5">
                            {{-- {{ single_price($order->grand_total) }} --}}
                            {{$order->combined_order_id != null ? $order->grand_total :  single_price($order->orderDetails->sum('price') + $order->orderDetails->sum('quantity')) }}
