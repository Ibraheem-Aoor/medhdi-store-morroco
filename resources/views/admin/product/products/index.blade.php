...
@section('title', translate('All Product'))
@section('content')
    ...
    <div class="card">
        ...
        as $key => $seller)
                ...
                @endisset>{{ translate('Num of Sale (High > Low)') }}</option>
        ...
        <option value="unit_price,desc"
            @isset($col_name, $query) @if ($col_name == 'unit_price' && $query == 'desc') selected @endif
        @endisset>{{ translate('Base Price (High > Low)') }}</option>
    <option value="unit_price,asc"
        @isset($col_name, $query) @if ($col_name == 'unit_price' && $query == 'asc') selected @endif
    @endisset>{{ translate('Base Price (Low > High)') }}</option>
</select>
</div>
<div class="col-md-2">
...
<tr>
    ...
    <th data-breakpoints="lg">{{ translate('New') }}</th>
    <th data-breakpoints="lg">{{ app()->getLocale() == 'en' ? 'Soon' : 'قريبا' }}</th>
...
@foreach ($products as $key => $product)
    <tr>
        ...
        <td>
            <label class="aiz-switch aiz-switch-success mb-0">
                <input onchange="update_new(this)" value="{{ $product->id }}"
                    type="checkbox" <?php if ($product->new == 1) {
                        echo 'checked';
                    } ?>>
                <span class="slider round"></span>
            </label>
        ...
        <td>
            <label class="aiz-switch aiz-switch-success mb-0">
                <input onchange="update_soon(this)" value="{{ $product->id }}"
                    type="checkbox" <?php if ($product->soon == 1) {
                        echo 'checked';
                    } ?>>
                <span class="slider round"></span>
            </label>
        ...
        <td>
            <label class="aiz-switch aiz-switch-success mb-0">
                <input onchange="update_best_selling(this)" value="{{ $product->id }}"
                    type="checkbox" <?php if ($product->best_selling == 1) {
                        echo 'checked';
                    } ?>>
                <span class="slider round"></span>
            </label>
        ...
        <td class="text-right">
            ...
            @else
                @if ($product->digital == 1)
                    @php
                        $route = 'digitalproducts.edit';

                    @endphp
                @else
                    @php
                        $route = 'products.admin.edit';
                    @endphp
                @endif
                <a class="btn btn-soft-primary btn-icon btn-circle btn-sm"
                    href="{{ route($route, ['id' => $product->id, 'lang' => env('DEFAULT_LANGUAGE')]) }}"
                    title="{{ translate('Edit') }}">
                    <i class="las la-edit"></i>
                </a>
            @endif
...
<script type="text/javascript">
    ...
    function update_new(el) {
        ...
        $.post('{{ route('products.new') }}', {
            ...
            if (data == 1) {
                AIZ.plugins.notify('success', '{{ translate('New')." " . translate('Label updated successfully') }}');
            } else {
                AIZ.plugins.notify('danger', '{{ translate('Something went wrong') }}');
            }
        });
    ...
    function update_soon(el) {
        if (el.checked) {
            var status = 1;
        } else {
            var status = 0;
        }
        $.post('{{ route('products.soon') }}', {
            _token: '{{ csrf_token() }}',
            id: el.value,
            status: status
        }, function(data) {
            if (data == 1) {
                AIZ.plugins.notify('success', '{{ translate('Soon Label updated successfully') }}');
            } else {
                AIZ.plugins.notify('danger', '{{ translate('Soon')." " . translate('Label updated successfully') }}');
            }
        });
    }
    ...
    function update_best_selling(el) {
        ...
        $.post('{{ route('products.best-selling') }}', {
            _token: '{{ csrf_token() }}',
            id: el.value,
            status: status
        }, function(data) {
            if (data == 1) {
                AIZ.plugins.notify('success', '{{ translate('best_selling')." " . translate('Label updated successfully') }}');
            } else {
                AIZ.plugins.notify('danger', '{{ translate('Something went wrong') }}');
            }
        });
