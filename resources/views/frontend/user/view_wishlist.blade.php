    ...
    <div class="row gutters-5">
        @forelse ($wishlists as $key => $wishlist)
            @if ($wishlist->product != null)
                <div class="col-lg-3 col-md-4 col-sm-6" id="wishlist_{{ $wishlist->id }}">
                    <div class="card mb-2 shadow-sm">
                        <div class="card-body p-3">
                            ...
                            <h5 class="fs-14 mb-0 lh-1-5 fw-600 text-truncate-2">
                                <a href="{{ route('product', $wishlist->product->slug) }}" class="text-reset" title="{{$wishlist->product->getTranslation('name')}}">{{ Str::limit($wishlist->product->getTranslation('name') , 30 , '...') }}</a>
                            </h5>
                        ...
                        <div class="card-footer px-2">
                            ...
                            <button type="button" class="btn btn-sm btn-block add-to-cart-btn ml-2" onclick="showAddToCartModal({{ $wishlist->product->id }})">
                                <i class="la fs-22 la-shopping-cart mr-2"></i>{{ translate('Add to cart')}}
                            </button>
                        </div>
                    </div>
                </div>
