...
<head>
    ...
    <!-- CSS Files -->
    <link rel="stylesheet" href="{{ asset('assets/css/vendors.css?v=0.03') }}">
    @if (\App\Models\Language::where('code', Session::get('locale', Config::get('app.locale')))->first()->rtl == 1)
        <link rel="stylesheet" href="{{ asset('assets/css/bootstrap-rtl.min.css') }}">
    @endif
    <link rel="stylesheet" href="{{ asset('assets/css/aiz-core.css?v=0.9') }}">
    <link rel="stylesheet" href="{{ asset('assets/css/swiper-bundle.css') }}">
    <link rel="stylesheet" href="{{ asset('assets/css/custom-style.css?v=0.4') }}">
    ...
    <style>
        ...
        body.loading {
            background: url('{{ asset('assets/img/Spinner-1s-200px (10).svg') }}') no-repeat 50% 30%;
...
<body>
    ...
    <div class="aiz-main-wrapper d-flex flex-column">
        ...
        <a class="whats-app"
            href="https://wa.me/+9720598298969"
            style="position: relative;" title="{{ translate('Contact Us') }}">
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <form class="form-default" role="form" id="buy_now_form" action="{{ route('payment.checkout') }}">
                @csrf
                ...
                <div class="modal-body">
                    <div class="p-3">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-10">
                                <input type="number" class="form-control mb-3" placeholder=""  name="phone" required>
                            </div>
                        </div>

                        <input type="checkbox" onchange="toggleCouponDiv(this);"> {{translate('I Have Coupn Code')}}

                        <div class="row" class="coupon-div d-none" id="coupon-div">
                            <div class="col-md-2">
                                <label>{{ translate('Coupon') }}</label>
                            </div>
                            <div class="col-md-10">
                                <input type="text" class="form-control mb-3" placeholder="{{translate('Coupon')}}"  id="coupon_code">
                                <button type="button" class="btn btn-success" onclick="event.preventDefault();apply_coupon_code();">{{translate('Apply')}}</button>
                            </div>
                        </div>
                    ...
                    <div class="form-group text-right">
                        <button type="submit" id="submit-form" class="btn btn-sm btn-primary">{{ translate('Save') }}</button>
                    </div>
                </div>
            </form>
...
{{--
    ...
    <script>
        function apply_coupon_code()
        {
            ...
            $.post('{{ route('checkout.apply_coupon_code') }}', {
    ...
    <script>
        $(document).on('submit', '#buy_now_form', function(e) {
                        e.preventDefault();
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                if (response.status && response.is_confirmed) {
                            ...
                            error: function(response) {
                                if (response.message) {
                                    ...
                                    $.each(response.responseJSON.errors , function(item , message){
                                        AIZ.plugins.notify('error', message);
                                    });
                                }
                            }
                        });
