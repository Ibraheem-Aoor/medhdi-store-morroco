...
@if (isset($category_id))
    @php
        $meta_title = 'منصة لوتو | تسوق اونلاين مع أسرع خدمة شراء وشحن من الصين';
...
@section('content')
  @php
    $angle  = (App()->getLocale() == 'en' )? 'right' : 'left';
  @endphp
                                    ...
                                    <div class="p-3">
                                        <ul class="list-unstyled">
                                            ...
                                            @foreach (\App\Models\Category::get() as $category)
                                                ...
                                                @endif
                                                <li class="mb-2">
                                                    <a class="text-reset fs-14 @if ($category_id == $category->id) fw-600 @endif"
                                                ...
                                                @foreach (\App\Utility\CategoryUtility::get_immediate_children_ids($category->id) as $key => $id)
                                                    <li class="ml-4 mb-2">
                                                        <a class="text-reset fs-14 @if ($category_id == $id) fw-600 @endif"
                            ...
                            <div class="row gutters-5 flex-wrap align-items-center">
                                ...
                                <div class="col-6 col-lg-auto mb-3 w-lg-200px">
                                    @if (Route::currentRouteName() != 'products.brand')
                                        <label class="mb-0 opacity-90 mb-1">{{ translate('Brands') }}</label>
                                        <select class="form-control form-control-sm aiz-selectpicker"
                                            ...
                                            @foreach (\App\Models\Brand::all() as $brand)
                                                <option value="{{ $brand->slug }}"
                                                    @isset($brand_id) @if ($brand_id == $brand->id) selected @endif
                                                @endisset>{{ $brand->getTranslation('name') }}</option>
                                        @endforeach
                                    </select>
                                @endif
                            </div>
                            <div class="col-6 col-lg-auto mb-3 w-lg-200px">
                                <label class="mb-0 opacity-90 mb-1">{{ translate('Sort by') }}</label>
                                <select class="form-control form-control-sm aiz-selectpicker" name="sort_by"
                                    onchange="filter()">
                                    <option value="newest"
                                        @isset($sort_by) @if ($sort_by == 'newest') selected @endif
                                    @endisset>{{ translate('Newest') }}</option>
                                <option value="oldest"
                                    @isset($sort_by) @if ($sort_by == 'oldest') selected @endif
                                @endisset>{{ translate('Oldest') }}</option>
                            <option value="price-asc"
                                @isset($sort_by) @if ($sort_by == 'price-asc') selected @endif
                            @endisset>{{ translate('Price low to high') }}</option>
                        <option value="price-desc"
                            @isset($sort_by) @if ($sort_by == 'price-desc') selected @endif
    ...
    <div class="row gutters-5 row-cols-xl-4 row-cols-xl-3 row-cols-lg-4 row-cols-md-3 row-cols-2">
